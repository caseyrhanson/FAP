...
#usage example: sbatch aaron_step3_samtoolsScan_mergechr.sh NBM-R-1
# Merges samples per chromosome. Create a for loop to merge each chromosome from each sample.
...
date
echo "This job runs step 3.2 (merge per chromosome calls) of Rupings VAP pipeline, one chromosome at a time."
...
#this step is for merging samtools files per chromosome.
perl /srv/gsfs0/projects/curtis/ruping/tools/seqare/DTrace.pl --configure $CONFIG --runlevel 4 --sampleName $SAMPLE --runTask mergeSamtoolsChr --germline samtoolsOnly --seqType paired-end,WGS --root $ROOT --threads $THREADS --somaticInfo $SOMATICINFO 2>>$ROOT/$SAMPLE.run.log
