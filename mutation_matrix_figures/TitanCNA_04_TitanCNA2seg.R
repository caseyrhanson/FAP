...
titancna2seg <- function(titanresult,titanparams,adjust_end=TRUE) {
  ...
  chr_sizes <- c(249250621,243199373,198022430,191154276,180915260,171115067,159138663,146364022,141213431,135534747,135006516,133851895,115169878,107349540,102531392,90354753,81195210,78077248,59128983,63025520,48129895,51304566)
...
cnainterval <- function(segments) {

  ##generate interval between two segments
  
  chrom_sizes <- read.table("/srv/gsfs0/projects/curtis/ref_genomes/broad.Homo_sapiens_assembly19.fa.sizes",as.is=TRUE)

  for (i in c(5:9,12)) {
    segments[[i]] <- as.character(segments[[i]])
  }
  ...
  for (i in 1:n_segments) {
    if (i == 1) {
      segment1 <- segments[i,]
      segment1$loc.start <- 1
      segment1$loc.end <- segments$loc.start[i] - 1
      segment1$num.mark <- 0
      for (j in c(5:9,12)) {
	segment1[1,j] <- paste("-",segments[i,j],sep=":")
      }
      segments <- rbind(segments,segment1)
      next
    }

    if (i == n_segments) {
      segment1 <- segments[i,]
      segment1$loc.start <- segments$loc.end[i] + 1
      segment1$loc.end <- chrom_sizes[[2]][match(segments$chrom[i],chrom_sizes[[1]])]
      segment1$num.mark <- 0
      for (j in c(5:9,12)) {
	segment1[1,j] <- paste(segments[i,j],"-",sep=":")
      }
    ...
    if (segments$chrom[i] == segments$chrom[i-1]) {
      segment1 <- segments[i-1,]
      segment1$loc.start <- segments$loc.end[i-1] + 1
      segment1$loc.end <- segments$loc.start[i] - 1
      segment1$num.mark <- 0
      for (j in c(5:9,12)) {
	segment1[1,j] <- paste(segments[c(i-1,i),j],collapse=":")
      }
      segments <- rbind(segments,segment1)
    }
    else {
	...
	segment1[1,j] <- paste("-",segments[i,j],sep=":")
      ...
      segment1 <- segments[i-1,]
      segment1$loc.start <- segments$loc.end[i-1] + 1
      segment1$loc.end <- chrom_sizes[[2]][match(segments$chrom[i-1],chrom_sizes[[1]])]
      segment1$num.mark <- 0
      for (j in c(5:9,12)) {
	segment1[1,j] <- paste(segments[i-1,j],"-",sep=":")
      }
      segments <- rbind(segments,segment1)
    }     
  }
  segments <- segments[order(match(segments$chrom,c(1:22,"X","Y")),segments$loc.start),]
