...
data.ccf$cancerdriver[data.ccf$geneName %in% drivers] = "Driver" #add string "Driver" to genes which are known drivers
  ...
  for (i in 1:length(samples)) {
    plot = ggplot(data = data.ccf,mapping = aes(x = Phylogeny))+
      ...
      ggrepel::geom_label_repel(data = data.ccf[which(data.ccf[,paste0(samples[i],"ccf")] > 0 & data.ccf$cancerdriver=="Driver"),],
                                aes_string(y = paste0(samples[i],"ccf"),label = "geneName"),size = 3)
    
    plot
    ggsave(filename = paste0("treeomics/",samples[i],"_treeomics_",mutlocation,"Mutations_Trunk_Shared_Private.pdf"),
           width = 12,height = 10)
  }
}
